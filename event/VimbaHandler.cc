...
#define CASE_VSN(t,v) case v:                                             \
  { const Pds::Vimba::t##ConfigV##v& c =                                  \
      *reinterpret_cast<const Pds::Vimba::t##ConfigV##v*>(tc->payload()); \
      return c.height(); }
...
static std::list<Pds::TypeId::Type> config_type_list()
{
  ...
  types.push_back(Pds::TypeId::Id_AlviumConfig);
  return types;
}
...
{
  std::list<Pds::TypeId::Type> types;
  types.push_back(Pds::TypeId::Id_VimbaFrame);
  return types;
}

VimbaHandler::VimbaHandler(const Pds::DetInfo& info, FeatureCache& cache) : 
...
unsigned VimbaHandler::nentries() const { return _entry ? 1 : 0; }

const Entry* VimbaHandler::entry(unsigned i) const { return i==0 ? _entry : 0; }

void VimbaHandler::rename(const char* s)
{
  if (_entry) _entry->desc().name(s);
}

void VimbaHandler::reset() 
{
  _entry = 0; 
  if (_pentry) { 
    delete _pentry; 
...
{
  if (type.id() == Pds::TypeId::Id_VimbaFrame)
  {
    const Pds::Vimba::FrameV1& f = *reinterpret_cast<const Pds::Vimba::FrameV1*>(payload);
    if (!_entry) return;

    const DescImage& desc = _entry->desc();
    unsigned o = desc.options();
    if (_options != o) {
      printf("VimbaHandler options %x -> %x\n",_options,o);
...
void VimbaHandler::_damaged() { if (_entry) _entry->invalid(); }

void VimbaHandler::_load_pedestals()
{
