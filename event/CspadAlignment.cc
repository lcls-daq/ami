...
//
//  Apply a clockwise rotation (in a LHS)
//
...
Ami::Cspad::QuadAlignment* QuadAlignment::load(FILE* g, bool offline)
{
  ...
  if (offline) {  // Read offline style file
      ...
      if (pname.size()) {
        if (oname == "SENS2X1:V1") {
          ...
          for(unsigned k=0; k<8; k++) {
            double dx=x0, dy=y0;
            _transform(dx,dy,
                       q[oindex]._twobyone[k]._pad.x,
                       q[oindex]._twobyone[k]._pad.y,
                       Rotation(unsigned(4.5-rot_z/90.)%NPHI));
            q[oindex]._twobyone[k]._pad.x = dx;
            q[oindex]._twobyone[k]._pad.y = dy;
            for(unsigned c=0; c<4; c++) {
              q[oindex]._twobyone[k]._corner[c].x = dx;
              q[oindex]._twobyone[k]._corner[c].y = dy;
            }
          }
        }
      }
    ...
    for(unsigned iq=0; iq<4; iq++) {
      for(unsigned i=0; i<8; ) { // loop over 2x1's
        ...
        q[iq]._twobyone[i]._pad.x = px;
        q[iq]._twobyone[i]._pad.y = py;
        for(unsigned j=0; j<4; j++) {
          q[iq]._twobyone[i]._corner[j].x=px;
          q[iq]._twobyone[i]._corner[j].y=py;
        }
        i++;
      }
    }
    ...
    */
    ...
    for(unsigned i=0; i<8; i++) {
      if (q[iq]._twobyone[i]._pad.x < qx) {
        if (q[iq]._twobyone[i]._pad.y + asicWidth < qy)
          si[i]=0;
        else if (q[iq]._twobyone[i]._pad.y < qy)
          si[i]=1;
        else if (q[iq]._twobyone[i]._pad.x + asicWidth < qx) 
          si[i]=2;
        else
          si[i]=3;
      }
    ...
    for(unsigned i=0; i<8; i++) {
      ...
      for(unsigned c=0; c<4; c++) {
        ...
        _transform(nq[iq]._twobyone[k]._corner[c].x,
                   ...
                   q[iq]._twobyone[i]._corner[c].x-bx,
                   q[iq]._twobyone[i]._corner[c].y-by,
                   qr);
      }
    }
  }
