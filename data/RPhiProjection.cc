  ...
  if (_input) {
	...
	if (_axis == R) {
	  ...
	  if (bounds(x0,x1,y0,y1,
	    ...
	    for(int j=y0; j<y1; j++) {
	      double dy  = double(j)-_yc;
	      double dy2 = dy*dy;
	      for(int k=x0; k<x1; k++) {
		double dx  = double(k)-_xc;
		double dx2 = dx*dx;
		double f   = atan2(dy,dx);
		if ( (f>=_lo && f<=_hi) ||
		     (f+2*M_PI <= _hi) )
		  o->addcontent(_input->content(k,j),sqrt(dx2+dy2));
	      }
	    }
	  }
	}
	...
	if (_axis == R) {
	  ...
	  if (bounds(x0,x1,y0,y1,
		...
		if ( (f>=_lo && f<=_hi) ||
		     (f+2*M_PI <= _hi) )
		  o->addy(_input->content(k,j),sqrt(dx2+dy2));
	      }
	    }
	  }
	}
