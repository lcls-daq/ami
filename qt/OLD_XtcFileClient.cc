#include "OLD_XtcFileClient.hh"

#include "ami/qt/OLD_FileSelect.hh"
...
namespace Ami {
  namespace Qt {
    ...
    public:
      ConfigureTask(OLD_XtcFileClient& c) : _c(c) {}
      ~ConfigureTask() {}
    public:
      void routine() { _c.configure(); delete this; }
    private:
      OLD_XtcFileClient& _c;
    };
  };
};
...
OLD_XtcFileClient::OLD_XtcFileClient(Ami::XtcClient& client,
			     const char* basedir) :
  QWidget (0,::Qt::Window),
  _client (client),
  _basedir(basedir),
  _task   (new Task(TaskObject("amiqt")))
{
  ...
  l->addWidget(_file_select = new OLD_FileSelect(0,QStringList()));
...
OLD_XtcFileClient::~OLD_XtcFileClient()
{
  _task->destroy();
}

void OLD_XtcFileClient::select_expt(const QString& expt)
{
  QStringList paths;
...
void OLD_XtcFileClient::configure_run()
{
  _task->call(new ConfigureTask(*this));
}

void OLD_XtcFileClient::configure()
{
...
void OLD_XtcFileClient::run() 
{
  _runB->setEnabled(false);
  _task->call(this); 
}

void OLD_XtcFileClient::ready() 
{
...
//
void OLD_XtcFileClient::routine()
{
