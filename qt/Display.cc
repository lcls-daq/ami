...
void Ami::Qt::Display::save_image()
{
  char time_buffer[32];
...
void Ami::Qt::Display::save_reference()
{
  QStringList list;
  for(std::list<QtBase*>::const_iterator it=_curves.begin(); it!=_curves.end(); it++) 
    list << (*it)->title();
...
void Ami::Qt::Display::add   (QtBase* b) 
{
...
void Ami::Qt::Display::xtransform_update()
{
  for(std::list<QtBase*>::iterator it=_curves.begin(); it!=_curves.end(); it++)
    (*it)->xscale_update();

  if (_curves.size())
    _xinfo->update(*_curves.front()->xinfo());
...
void Ami::Qt::Display::xrange_change()
{
  if (_xrange->isAuto()) 
...
void Ami::Qt::Display::yrange_change()
{
  if (_yrange->isAuto()) 
...
const Ami::AbsTransform& Ami::Qt::Display::xtransform() const { return *_xtransform; }

const std::list<QtBase*> Ami::Qt::Display::plots() const { return _curves; }

const AxisArray& Ami::Qt::Display::xinfo() const { return *_xinfo; }

PlotFrame* Ami::Qt::Display::plot() const { return _plot; }
